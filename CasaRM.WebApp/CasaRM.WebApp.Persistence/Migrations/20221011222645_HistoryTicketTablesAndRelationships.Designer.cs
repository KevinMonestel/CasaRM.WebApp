// <auto-generated />
using System;
using CasaRM.WebApp.Persistance;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace CasaRM.WebApp.Persistence.Migrations
{
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20221011222645_HistoryTicketTablesAndRelationships")]
    partial class HistoryTicketTablesAndRelationships
    {
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "6.0.9")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder, 1L, 1);

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.ApplicationUser", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<int>("AccessFailedCount")
                        .HasColumnType("int");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Email")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<bool>("EmailConfirmed")
                        .HasColumnType("bit");

                    b.Property<bool>("LockoutEnabled")
                        .HasColumnType("bit");

                    b.Property<DateTimeOffset?>("LockoutEnd")
                        .HasColumnType("datetimeoffset");

                    b.Property<string>("NormalizedEmail")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedUserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("PasswordHash")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("PhoneNumber")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("PhoneNumberConfirmed")
                        .HasColumnType("bit");

                    b.Property<string>("SecurityStamp")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool>("TwoFactorEnabled")
                        .HasColumnType("bit");

                    b.Property<string>("UserName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedEmail")
                        .HasDatabaseName("EmailIndex");

                    b.HasIndex("NormalizedUserName")
                        .IsUnique()
                        .HasDatabaseName("UserNameIndex")
                        .HasFilter("[NormalizedUserName] IS NOT NULL");

                    b.ToTable("Usuarios", (string)null);
                });

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.CompanionData", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("Age")
                        .HasColumnType("int")
                        .HasColumnName("Edad");

                    b.Property<DateTime>("DateOfBirth")
                        .HasColumnType("datetime2")
                        .HasColumnName("FechaNacimiento");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)")
                        .HasColumnName("NombreCompleto");

                    b.Property<string>("Gender")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("Sexo");

                    b.Property<string>("Identification")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)")
                        .HasColumnName("Identificacion");

                    b.Property<string>("Illness")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("Enfermedad");

                    b.Property<bool>("IsAPersonInCharge")
                        .HasColumnType("bit")
                        .HasColumnName("EsPersonaACargo");

                    b.Property<string>("MaritalStatus")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("EstadoCivil");

                    b.Property<string>("Nacionality")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("Nacionalidad");

                    b.Property<string>("Observations")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)")
                        .HasColumnName("Observaciones");

                    b.Property<string>("Ocupation")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("Ocupacion");

                    b.Property<string>("PhonesNumbers")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("Telefonos");

                    b.Property<string>("Relationship")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)")
                        .HasColumnName("Parentesco");

                    b.Property<string>("Scholarship")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("Escolaridad");

                    b.Property<int>("SocialStudyId")
                        .HasColumnType("int")
                        .HasColumnName("EstudioSocialId");

                    b.Property<string>("Treatment")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)")
                        .HasColumnName("Tratamiento");

                    b.Property<string>("WorkAddress")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("LugarDeTrabajo");

                    b.HasKey("Id");

                    b.HasIndex("SocialStudyId");

                    b.ToTable("DatosAcompannantes");
                });

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.Contribution", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)")
                        .HasColumnName("NombreCompleto");

                    b.Property<decimal>("MonthlyContribution")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("AporteMensual");

                    b.Property<string>("Ocupation")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("Ocupacion");

                    b.Property<string>("Relationship")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)")
                        .HasColumnName("Parentesco");

                    b.Property<int>("SocialStudyId")
                        .HasColumnType("int")
                        .HasColumnName("EstudioSocialId");

                    b.HasKey("Id");

                    b.HasIndex("SocialStudyId");

                    b.ToTable("Aportes");
                });

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.FamilyGroup", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("Age")
                        .HasColumnType("int")
                        .HasColumnName("Edad");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)")
                        .HasColumnName("NombreCompleto");

                    b.Property<string>("MaritalStatus")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("EstadoCivil");

                    b.Property<decimal>("MonthlyGrossIncome")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("IngresoBrutoMensual");

                    b.Property<string>("Ocupation")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("Ocupacion");

                    b.Property<string>("Relationship")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)")
                        .HasColumnName("Parentesco");

                    b.Property<string>("Scholarship")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("Escolaridad");

                    b.Property<int>("SocialStudyId")
                        .HasColumnType("int")
                        .HasColumnName("EstudioSocialId");

                    b.HasKey("Id");

                    b.HasIndex("SocialStudyId");

                    b.ToTable("GruposFamiliares");
                });

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.HistoryTicket", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("BabyTrolleyQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadCoches");

                    b.Property<int>("BathClothsQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadPannosBanno");

                    b.Property<int>("BlackoutCurtainsQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadCortinasBlackOut");

                    b.Property<int>("BlanketsQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadCobijas");

                    b.Property<int>("ClosetsQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadClosets");

                    b.Property<int>("CradleQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadCunas");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2")
                        .HasColumnName("CreadoEn");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)")
                        .HasColumnName("CreadoPor");

                    b.Property<int>("DecorationsQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadDecoraciones");

                    b.Property<int>("DecorativePicturesQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadCuadrosDecorativos");

                    b.Property<int>("ElasticSheetsQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadSabanasElastico");

                    b.Property<int>("EnclosuresQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadEncierros");

                    b.Property<int>("HandClothsQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadPannosMano");

                    b.Property<int>("HospitalBedsQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadCamasHospitalarias");

                    b.Property<bool>("JourneyMade")
                        .HasColumnType("bit")
                        .HasColumnName("RecorridoRelizado");

                    b.Property<int>("NightstandQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadMesasDeNoche");

                    b.Property<string>("Observations")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)")
                        .HasColumnName("Observaciones");

                    b.Property<int>("PillowCasesQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadFundasAlmohadas");

                    b.Property<int>("PillowCoversQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadCobertoresAlmohadas");

                    b.Property<int>("PillowsQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadAlmohadas");

                    b.Property<int>("QuiltsQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadEdredones");

                    b.Property<int>("SheetsQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadSabanas");

                    b.Property<int>("SidewalkQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadBanquetas");

                    b.Property<int>("SingleBedsQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadCamasIndividuales");

                    b.Property<int>("ToiletKitQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadKitsAseo");

                    b.Property<int>("TrundleBedsQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadCamasNido");

                    b.Property<int>("TubQty")
                        .HasColumnType("int")
                        .HasColumnName("CantidadTinas");

                    b.Property<bool>("keysDelivered")
                        .HasColumnType("bit")
                        .HasColumnName("LlavesEntregadas");

                    b.HasKey("Id");

                    b.HasIndex("CreatedBy");

                    b.ToTable("BoletaHistorial");
                });

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.Host", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2")
                        .HasColumnName("CreadoEn");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)")
                        .HasColumnName("CreadoPor");

                    b.Property<int>("SocialStudyId")
                        .HasColumnType("int")
                        .HasColumnName("EstudioSocialId");

                    b.Property<DateTime>("UpdatedAt")
                        .HasColumnType("datetime2")
                        .HasColumnName("ActualizadoEn");

                    b.HasKey("Id");

                    b.HasIndex("CreatedBy");

                    b.HasIndex("SocialStudyId");

                    b.ToTable("Huespedes");
                });

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.HostingHistory", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<DateTime>("CreatedAt")
                        .HasColumnType("datetime2")
                        .HasColumnName("CreadoEn");

                    b.Property<string>("CreatedBy")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)")
                        .HasColumnName("CreadoPor");

                    b.Property<DateTime?>("EndDate")
                        .HasColumnType("datetime2")
                        .HasColumnName("FechaSalida");

                    b.Property<int?>("HistoryTicketDeliveryId")
                        .HasColumnType("int")
                        .HasColumnName("BoletaHistorialEntregaId");

                    b.Property<int?>("HistoryTicketReceptionId")
                        .HasColumnType("int")
                        .HasColumnName("BoletaHistorialRecibimientoId");

                    b.Property<Guid>("HostId")
                        .HasColumnType("uniqueidentifier")
                        .HasColumnName("HuespedId");

                    b.Property<int>("RoomNumber")
                        .HasColumnType("int")
                        .HasColumnName("NumeroHabitacion");

                    b.Property<DateTime>("StartDate")
                        .HasColumnType("datetime2")
                        .HasColumnName("FechaEntrada");

                    b.HasKey("Id");

                    b.HasIndex("CreatedBy");

                    b.HasIndex("HistoryTicketDeliveryId");

                    b.HasIndex("HistoryTicketReceptionId");

                    b.HasIndex("HostId");

                    b.ToTable("HistorialesHospedajes");
                });

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.HousingSituation", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("BasicServices")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("ServiciosBasicos");

                    b.Property<int>("BathroomsQuantity")
                        .HasColumnType("int")
                        .HasColumnName("CantidadBannos");

                    b.Property<int>("BedroomsQuantity")
                        .HasColumnType("int")
                        .HasColumnName("CantidadDormitorios");

                    b.Property<string>("HousingConservationStatus")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("EstadoConservacionVivienda");

                    b.Property<string>("HousingConstructionMaterialsFloor")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("MaterialConstruccionPiso");

                    b.Property<string>("HousingConstructionMaterialsWall")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("MaterialConstruccionParedes");

                    b.Property<string>("HousingTenureCondition")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("CondicionTenenciaVivienda");

                    b.Property<int>("RoomsQuantity")
                        .HasColumnType("int")
                        .HasColumnName("CantidadAposentos");

                    b.HasKey("Id");

                    b.ToTable("SituacionesHabitacionales");
                });

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.MinorPersonData", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("Age")
                        .HasColumnType("int")
                        .HasColumnName("Edad");

                    b.Property<string>("AttentionService")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)")
                        .HasColumnName("ServicioAtencion");

                    b.Property<string>("Canton")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("Canton");

                    b.Property<DateTime>("DateOfBirth")
                        .HasColumnType("datetime2")
                        .HasColumnName("FechaNacimiento");

                    b.Property<string>("Diagnosis")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)")
                        .HasColumnName("Diagnostico");

                    b.Property<string>("District")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("Distrito");

                    b.Property<string>("ExactDirection")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)")
                        .HasColumnName("DireccionExacta");

                    b.Property<string>("FileNumber")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)")
                        .HasColumnName("NumeroExpediente");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)")
                        .HasColumnName("NombreCompleto");

                    b.Property<string>("Gender")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("Sexo");

                    b.Property<string>("Nacionality")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("Nacionalidad");

                    b.Property<string>("Observations")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)")
                        .HasColumnName("Observaciones");

                    b.Property<string>("Province")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("Provincia");

                    b.Property<string>("Scholarship")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("Escolaridad");

                    b.Property<string>("SocialSecurityType")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("TipoSeguroSocial");

                    b.Property<string>("TreatingPhysician")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)")
                        .HasColumnName("MedicoTratante");

                    b.Property<string>("Treatment")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)")
                        .HasColumnName("Tratamiento");

                    b.HasKey("Id");

                    b.ToTable("DatosPersonasMenores");
                });

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.SocialStudy", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<int>("HousingSituationId")
                        .HasColumnType("int")
                        .HasColumnName("SituacionHabitacionalId");

                    b.Property<int>("MinorPersonDataId")
                        .HasColumnType("int")
                        .HasColumnName("DatosPersonaMenorId");

                    b.Property<decimal>("PerCapitaIncome")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("IngresoPerCapita");

                    b.Property<string>("PovertyCategory")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)")
                        .HasColumnName("CategoriaPobreza");

                    b.Property<string>("Recomendation")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)")
                        .HasColumnName("Recomendaciones");

                    b.Property<string>("SignatureDataUrl")
                        .IsRequired()
                        .HasColumnType("nvarchar(max)")
                        .HasColumnName("DatosFirmaUrl");

                    b.Property<string>("SupportServices")
                        .IsRequired()
                        .HasMaxLength(250)
                        .HasColumnType("nvarchar(250)")
                        .HasColumnName("RedesApoyo");

                    b.Property<decimal>("TotalRevenue")
                        .HasColumnType("decimal(18,2)")
                        .HasColumnName("TotalIngresos");

                    b.HasKey("Id");

                    b.HasIndex("HousingSituationId");

                    b.HasIndex("MinorPersonDataId");

                    b.ToTable("EstudiosSociales");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRole", b =>
                {
                    b.Property<string>("Id")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("ConcurrencyStamp")
                        .IsConcurrencyToken()
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("Name")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.Property<string>("NormalizedName")
                        .HasMaxLength(256)
                        .HasColumnType("nvarchar(256)");

                    b.HasKey("Id");

                    b.HasIndex("NormalizedName")
                        .IsUnique()
                        .HasDatabaseName("RoleNameIndex")
                        .HasFilter("[NormalizedName] IS NOT NULL");

                    b.ToTable("Roles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("RoleId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("RoleId");

                    b.ToTable("ClaimsRoles", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.Property<int>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int");

                    SqlServerPropertyBuilderExtensions.UseIdentityColumn(b.Property<int>("Id"), 1L, 1);

                    b.Property<string>("ClaimType")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("ClaimValue")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("Id");

                    b.HasIndex("UserId");

                    b.ToTable("ClaimsUsuario", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.Property<string>("LoginProvider")
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.Property<string>("ProviderKey")
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.Property<string>("ProviderDisplayName")
                        .HasColumnType("nvarchar(max)");

                    b.Property<string>("UserId")
                        .IsRequired()
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("LoginProvider", "ProviderKey");

                    b.HasIndex("UserId");

                    b.ToTable("LoginsUsuario", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("RoleId")
                        .HasColumnType("nvarchar(450)");

                    b.HasKey("UserId", "RoleId");

                    b.HasIndex("RoleId");

                    b.ToTable("RolesUsuario", (string)null);
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.Property<string>("UserId")
                        .HasColumnType("nvarchar(450)");

                    b.Property<string>("LoginProvider")
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.Property<string>("Name")
                        .HasMaxLength(128)
                        .HasColumnType("nvarchar(128)");

                    b.Property<string>("Value")
                        .HasColumnType("nvarchar(max)");

                    b.HasKey("UserId", "LoginProvider", "Name");

                    b.ToTable("TokensUsuario", (string)null);
                });

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.CompanionData", b =>
                {
                    b.HasOne("CasaRM.WebApp.Persistence.Models.SocialStudy", "SocialStudy")
                        .WithMany("CompanionData")
                        .HasForeignKey("SocialStudyId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("SocialStudy");
                });

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.Contribution", b =>
                {
                    b.HasOne("CasaRM.WebApp.Persistence.Models.SocialStudy", "SocialStudy")
                        .WithMany("Contribution")
                        .HasForeignKey("SocialStudyId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("SocialStudy");
                });

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.FamilyGroup", b =>
                {
                    b.HasOne("CasaRM.WebApp.Persistence.Models.SocialStudy", "SocialStudy")
                        .WithMany("FamilyGroup")
                        .HasForeignKey("SocialStudyId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("SocialStudy");
                });

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.HistoryTicket", b =>
                {
                    b.HasOne("CasaRM.WebApp.Persistence.Models.ApplicationUser", "ApplicationUser")
                        .WithMany()
                        .HasForeignKey("CreatedBy")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ApplicationUser");
                });

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.Host", b =>
                {
                    b.HasOne("CasaRM.WebApp.Persistence.Models.ApplicationUser", "ApplicationUser")
                        .WithMany("Host")
                        .HasForeignKey("CreatedBy")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("CasaRM.WebApp.Persistence.Models.SocialStudy", "SocialStudy")
                        .WithMany()
                        .HasForeignKey("SocialStudyId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ApplicationUser");

                    b.Navigation("SocialStudy");
                });

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.HostingHistory", b =>
                {
                    b.HasOne("CasaRM.WebApp.Persistence.Models.ApplicationUser", "ApplicationUser")
                        .WithOne()
                        .HasForeignKey("CasaRM.WebApp.Persistence.Models.HostingHistory", "CreatedBy")
                        .OnDelete(DeleteBehavior.NoAction)
                        .IsRequired();

                    b.HasOne("CasaRM.WebApp.Persistence.Models.HistoryTicket", "HistoryTicketDelivery")
                        .WithMany()
                        .HasForeignKey("HistoryTicketDeliveryId");

                    b.HasOne("CasaRM.WebApp.Persistence.Models.HistoryTicket", "HistoryTicketReception")
                        .WithMany()
                        .HasForeignKey("HistoryTicketReceptionId");

                    b.HasOne("CasaRM.WebApp.Persistence.Models.Host", "Host")
                        .WithMany()
                        .HasForeignKey("HostId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("ApplicationUser");

                    b.Navigation("HistoryTicketDelivery");

                    b.Navigation("HistoryTicketReception");

                    b.Navigation("Host");
                });

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.SocialStudy", b =>
                {
                    b.HasOne("CasaRM.WebApp.Persistence.Models.HousingSituation", "HousingSituation")
                        .WithMany()
                        .HasForeignKey("HousingSituationId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("CasaRM.WebApp.Persistence.Models.MinorPersonData", "MinorPersonData")
                        .WithMany()
                        .HasForeignKey("MinorPersonDataId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.Navigation("HousingSituation");

                    b.Navigation("MinorPersonData");
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityRoleClaim<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserClaim<string>", b =>
                {
                    b.HasOne("CasaRM.WebApp.Persistence.Models.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserLogin<string>", b =>
                {
                    b.HasOne("CasaRM.WebApp.Persistence.Models.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserRole<string>", b =>
                {
                    b.HasOne("Microsoft.AspNetCore.Identity.IdentityRole", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();

                    b.HasOne("CasaRM.WebApp.Persistence.Models.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("Microsoft.AspNetCore.Identity.IdentityUserToken<string>", b =>
                {
                    b.HasOne("CasaRM.WebApp.Persistence.Models.ApplicationUser", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired();
                });

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.ApplicationUser", b =>
                {
                    b.Navigation("Host");
                });

            modelBuilder.Entity("CasaRM.WebApp.Persistence.Models.SocialStudy", b =>
                {
                    b.Navigation("CompanionData");

                    b.Navigation("Contribution");

                    b.Navigation("FamilyGroup");
                });
#pragma warning restore 612, 618
        }
    }
}
